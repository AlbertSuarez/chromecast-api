...
from src import RETRIEVE_RETRY, MESSAGE_PLAY_SUCCESS, MESSAGE_UNEXPECTED_ERROR, RETRIEVE_RTD, \
    MESSAGE_PLAY_DEVICE_NOT_FOUND, MIME_TYPES, MESSAGE_PLAY_MIME_DEVICE_NOT_SUPPORTED, PLAY_RANDOM
from src.api.services import response, random_extractor
...
def get(name, source_url):
    ...
    try:
        ...
        source_name = source_url.split('/')[-1] if not random_video else random_video.get('title')
